...
function renderCars() {
    ...
    catalog.innerHTML = filtered.length > 0 
        ? filtered.map(car => `
            <div class="car-card">
                <h3>${car.name}</h3>
                <p>Стоимость: <strong>${car.price}</strong></p>
                <button onclick="alert('Заявка на ${car.name} принята!')">Купить сейчас</button>
            </div>
        `).join('')
        : '<p style="text-align:center; color: #666;">Ничего не найдено. Попробуйте изменить запрос.</p>';
}

// Первый запуск отображения
renderCars();
